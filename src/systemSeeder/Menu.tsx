...
const Menu = ({ SeedingValues, setSeedingValues }: Props) => {
  ...
  const [rotateFactorInEdit, setRotateFactorInEdit] = useState(
    SeedingValues.rotationFactor,
  );
  const [numberOfBodiesInEdit, setNumberOfBodiesInEdit] = useState(
    SeedingValues.numberOfBodies,
  );
  const [gravityConstantInEdit, setGravityConstantInEdit] = useState(
    SeedingValues.gravityConstant,
  );
  ...
  return (
    ...
    >
      ...
      Cloud Rotation Rate
      <Slider
        value={rotateFactorInEdit}
        onMouseDown={(e) => e.stopPropagation()}
        onChange={(_, v) => {
          setRotateFactorInEdit(v as number);
        }}
        min={0}
        max={1}
        step={0.01}
      ...
      Number of Bodies
      <Slider
        value={numberOfBodiesInEdit}
        onMouseDown={(e) => e.stopPropagation()}
        onChange={(_, v) => {
          setNumberOfBodiesInEdit(v as number);
        }}
        min={3}
        max={2000}
      />
      Gravity Constant
      <Slider
        value={gravityConstantInEdit}
        onMouseDown={(e) => e.stopPropagation()}
        onChange={(_, v) => {
          setGravityConstantInEdit(v as number);
        }}
        min={0}
        max={0.01}
        step={0.0001}
